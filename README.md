...
It started from a real problem: a Meta AI security researcher named Summer Yue gave her agent a simple instruction — *"organize my inbox, but ask before deleting anything."* The agent didn't ignore her.
It **lost the instruction** under context compression, silently deleted her emails, and she had to physically run to her computer to stop it.
...
### 📱 Mobile Dashboard
A full-featured web dashboard accessible from any device on the same network.
No app store, no installation — just open the browser.
...
### 🗞 AI News Feed
Automatic AI news aggregation from Google News RSS — searches in Turkish and English.
Delivered to dashboard and Telegram at 13:00 and 20:00 daily via cron.

### 🤖 Theia Chatbot
Claude-powered conversational interface. Knows the full Theia Guard story, Summer Yue incident, OpenClaw architecture.
Runs through a local API proxy — your key never leaves your machine.
...
### 🧠 Telegram Memory & Reminder Sync
A unified memory and alert system directly integrated with the Telegram bot. Built on the "Underthinking" philosophy to separate passive archiving from active alerting.

- `/not [message]`: **Passive Memory.** Saves the note silently to `notes.json` for Dashboard viewing. No notification.
  *(Example: `/not Arabayı A blok 3. kata park ettim`)*
- `/h [minutes] [message]`: **Active Reminder.** Acts as a timed trigger. Sends a push notification to Telegram exactly when the time is up.
  *(Example: `/h 45 Fırını kapat`)*
...
├── telegram_approval.py     # Telegram bot approval & memory channel
...
├── notes.json               # Passive memory storage (auto-generated)
...
### Setup
```bash
git clone [https://github.com/ismailkarabulut-lang/theia-guard](https://github.com/ismailkarabulut-lang/theia-guard)
cd theia-guard
...
### Run (Fully Synchronized)
To run the API (Memory) and the Telegram Bot (Senses) simultaneously in perfect sync, avoiding port conflicts and detached states:

```bash
# Terminal 1 — Start the entire base station (API & Bot)
cd ~/Masaüstü/theia-guard-core && (python3 api_server.py & python3 telegram_approval.py)

# Terminal 2 — Approval Gate (Optional, for command execution)
tg
```
...
| 9 | Notes & Planning tab (Telegram Sync) | ✅ Complete |
...
That conversation happened at 11 PM. By morning, there was a working approval gate.
By the next evening, a mobile dashboard with real-time notifications.
...
> Millions of people are curious about AI. Open source agent capabilities feel like rocket science to most of them.

The goal is to bring that down to the level of opening an app.
